...
export default function Hero({scrollToAbout, heroRef, scrollToProjects, scrollToContacts}) {
    ...
    return (
        <section ref={heroRef} className="text-center w-7/8 mx-auto mt-20 flex flex-col items-center justify-center max-h-max py-30">
            <button className="bg-slate-900 text-white bar opacity-0 px-5 py-2 rounded-full">
                👋 Hello I am
            </button>
            <h1 className="text-[6rem] bar opacity-0 text-blue-950 font-semibold name">
                Kent Nicholas
            </h1>
            <p className="text-3xl bar opacity-0 font-semibold i-am">
                I am a{" "}
                <span className="text-blue-600 underline underline-offset-10">
                    {words[index].substring(0, substring)}
                </span>
            </p>
            <p className="w-3/4 mx-auto mt-10 text-lg text-gray-600 bar opacity-0 description">
            ...
            <div className="font-semibold flex mt-5 items-center justify-center gap-5 max-h-max w-full mx-auto button-container">
                <button onClick={scrollToProjects} className="bg-black text-white bar opacity-0 hover:scale-105 buttons py-2 px-5 rounded-md hover:bg-blue-600 transition-all duration-300">
                    View My Work
                </button>
                <button onClick={scrollToContacts} className="hover:scale-105 border bar opacity-0 py-2 px-5 rounded-md buttons hover:text-white hover:bg-blue-600 transition-all duration-300 ">
                    Let's Connect!
                </button>
            </div>
            <div className=" mx-auto flex gap-10 max-w-max max-h-max mt-5">
                <button className="bar opacity-0 shadow-link transition-all duration-300 hover:scale-110 bg-white p-4 rounded-full">
                    <Github />
                </button>
                <button className="bar opacity-0 shadow-link transition-all duration-300 hover:scale-110 bg-white p-4 rounded-full">
                    <Facebook />
            ...
            <div className="flex items-center gap-32 hero-footer max-w-max max-h-max mx-auto mt-10">
                ...
                <div>
                    ...
                    <p className="text-gray-500 bar opacity-0">Projects Completed</p>
                </div>
                <div>
                    <h1 className="bar opacity-0 font-bold text-2xl">100%</h1>
                    <p className="text-gray-500 opacity-0 bar">Client Satisfaction</p>
                </div>
            </div>
